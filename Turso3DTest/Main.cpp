...
private:
    ...
    AutoPtr<FrameBuffer> viewFbo;
    AutoPtr<FrameBuffer> viewMRTFbo;
    AutoPtr<Texture> colorBuffer;
    AutoPtr<Texture> normalBuffer;
    AutoPtr<Texture> depthStencilBuffer;
    AutoPtr<Texture> noiseTexture;

    AutoPtr<FrameBuffer> ssaoFbo;
    AutoPtr<FrameBuffer> msaaResolveSrcFbo;
    AutoPtr<FrameBuffer> msaaResolveDestFbo;
    AutoPtr<Texture> normalResolveBuffer;
    AutoPtr<Texture> depthResolveBuffer;
    AutoPtr<Texture> ssaoTexture;
...
int TestApplication::Run(const std::vector<std::string>& arguments)
{
    ...
    viewMRTFbo = new FrameBuffer();
    colorBuffer = new Texture();
    normalBuffer = new Texture();
    depthStencilBuffer = new Texture();

    // Extra buffers needed for SSAO and multisampling resolve when using SSAO with multisampling
    ssaoFbo = new FrameBuffer();
    msaaResolveSrcFbo = new FrameBuffer();
    msaaResolveDestFbo = new FrameBuffer();
    normalResolveBuffer = new Texture();
    depthResolveBuffer = new Texture();
    ...
    // Fill initial scene content (first preset)
    CreateScene(0);
...
void TestApplication::CreateScene(int preset)
{
    ...
    if (preset == 0)
    {
...

        {
            ...
            object->SetScale(Vector3(1008.f, 50.0f, 1.0f));
            object->SetModel(cache->LoadResource<Model>("Box.mdl"));
            object->SetMaterial(cache->LoadResource<Material>("Stone.json"));
            object->SetCastShadows(true);
        }

        {
            ...
            object->SetStatic(true);
            object->SetPosition(Vector3(0.0f, 25.0f, 0.0f));
            object->SetScale(Vector3(1.0f, 50.0f, 1008.0f));
            object->SetModel(cache->LoadResource<Model>("Box.mdl"));
            object->SetMaterial(cache->LoadResource<Material>("Stone.json"));
            object->SetCastShadows(true);
        }
...
void TestApplication::PrepareRender()
{
    ZoneScoped;
    ...
    // Set correct aspect ratio according to current window size
...
void TestApplication::RenderAndPresent()
{
    ZoneScoped;
